...
afterAll(async() => {
  ...
  // START TO TESTING
  describe("input form dengan PrivyID",() => {

    test("checklist tnc", async() => { 
      await page.goto('https://britouch.mrmbdg.com/apply',{waitUntil: 'networkidle2'});
      ...
      await page.waitForSelector('a[class="btn btn-bri btn-bri__privy btn-cta text-center btn-bri__privy-centered"]', 
      {
        visible : true,
        timeout: 60000,
        waitUntil : "domcontentloaded"
      });
      await page.click('a[class="btn btn-bri btn-bri__privy btn-cta text-center btn-bri__privy-centered"]');
      await page.waitForSelector('input[name="user[privyId]"]', 
      {
        visible : true,
        timeout: 60000,
        waitUntil : "domcontentloaded"
      });
      await page.type('input[name="user[privyId]"]', 'EL8398');
      await page.click('button[type="submit"]');
      await page.waitForSelector('input[name="user[secret]"]', 
      {
        visible : true,
        timeout: 60000,
        waitUntil : "domcontentloaded"
      });
      await page.type('input[name="user[secret]"]', '1y4l221');
      await page.click('button[type="submit"]');
    },200000)

    test('fill form step one', async() =>{

      await page.waitForSelector('input[name="nama_lengkap"]', 
      {
        visible : true,
        timeout: 60000,
        waitUntil : "domcontentloaded"
